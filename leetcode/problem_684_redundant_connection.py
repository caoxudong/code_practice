...
class Solution:
    def _is_connected(self, src, target, visited, adj_list):
        stack: list[int] = []
        sub_nodes = adj_list[src]
        for tmp_sub_node in sub_nodes:
            stack.append(tmp_sub_node)
        while len(stack) > 0:
            popped_node = stack.pop()
            if popped_node == target:
                return True
            else:
                popped_node_sub_nodes = adj_list[popped_node]
                for tmp_pop_sub_node in popped_node_sub_nodes:
                    stack.append(tmp_pop_sub_node)
        return False
